...
class TradingBotBacktestDB:
    def __init__(self, tradingbot, is_backtest):
        ...
        if self.has_backtest_table():
            ...
            self.table_def_keys = tradingbot.append_specific_param_columns()
    ...
    def create_backtest_management_table(self):
        ...
        table_def = self.append_backtest_specific_params_columns_to_table_def(
            table_def)
    ...
    def alter_add_foreign_key_to_backtest_summary(self):
        ...
        with op.batch_alter_table(self.backtest_management_table_name) as batch_op:
            batch_op.create_foreign_key("fk_management_summary", "backtest_summary", [
    ...
    def update_summary(self, transaction_logs, summary_id):
        ...
        total_return_percentage = self.calc_total_return_percentage(
            self.transaction_logs_df)
        ...
        filled_none_summary_dict = {k: None if str(v) == "nan" and np.isnan(
            v) else v for k, v, in summary_dict.items()}
        self.db_client.session.bulk_update_mappings(
            BacktestSummary, [filled_none_summary_dict])
    ...
    def get_row_with_condition(self, profit_status=None, order_type=None):
        profit_status_condition = (
            self.transaction_logs_df["profit_status"] == profit_status)
        order_type_condition = (
            self.transaction_logs_df["order_type"] == order_type)
    ...
    def build_drawdowns(self):
        ...
        for log in self.transaction_logs_df.itertuples():
            if max_balance < log.current_balance:
                ...
                current_relative_drawdown = (
                    abs(current_drawdown) / max_balance)*100
        ...
        for row_id, row in enumerate(profit_status_np):
            if row[0] == profit_status:
                ...
                if consective_flag:
                    ...
                    consecutive_win_lose_entries.append(
    ...
    def insert_params_management(self, summary_id):
        ...
        self.tradingbot.combined_params["backtest_summary_id"] = int(
            summary_id)

        self.db_client.connector.execute(
            backtest_management.insert().values(self.tradingbot.combined_params))

    def bulk_insert(self):
        self.tradingbot.db_client.session.commit()
